// Package cli builds the cses command tree on top of the cses library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: cses.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cses",
		Short: "Browse CSES competitive programming problem set",
		Long: `cses browses the CSES Problem Set at https://cses.fi/problemset/ — a curated
collection of ~300 competitive programming problems organized by category.

No account or API key is required. Output can be formatted as a table, JSON,
JSONL, CSV, TSV, or URL list.

cses is an independent tool and is not affiliated with CSES or its authors.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVarP(&app.output, "output", "o", "auto", "output format: table|json|jsonl|csv|tsv|url (auto=table on TTY, jsonl piped)")
