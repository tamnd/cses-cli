...
describes what users get. Pick a format with `-o`, or let cses choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cses <command> -o table   # aligned columns for reading
cses <command> -o jsonl   # one JSON object per line, for piping
cses <command> -o json    # a single JSON array
cses <command> -o csv     # spreadsheet friendly
cses <command> -o tsv     # tab-separated
cses <command> -o url     # just the URL column
cses <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cses <command> --fields id,title,url
```
...
```bash
cses <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cses <command>            # a table, because this is a terminal
cses <command> | wc -l    # JSONL, because this is a pipe
```
